...
export const calculate = (expression, debug = false) => {
...
export const prepareEquation = (equation, x) => {
    const arrEquation = equation.split('=');

    if (arrEquation.length != 2) {
        throw new Error(`Equation error, need start eith Y=<equation>`);
    }
    const eq = arrEquation[1].split('x');

    for (let i in eq) {
        if (i < eq.length - 1) {
            eq[i] = `${eq[i]}${isNaN(eq[i][eq[i].length - 1]) ? `${x}` : `*${x}`}`
        }
    }
